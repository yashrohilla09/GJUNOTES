...
let CURRICULUM = CURRICULUM_DEFAULT;
...
class Database {
    static async init() {
        ...
        this.initializingPromise = (async () => {
            if (window.isFirebaseInitialized && window.db) {
                try {
                    // 1. Real-time Curriculum Sync Listener
                    db.collection("curriculum").doc("default").onSnapshot((doc) => {
                        if (doc.exists) {
                            CURRICULUM = doc.data().semesters;
                        } else {
                            db.collection("curriculum").doc("default").set({ semesters: CURRICULUM_DEFAULT })
                                .catch(e => console.error("Error seeding curriculum:", e));
                            CURRICULUM = CURRICULUM_DEFAULT;
                        }
                        triggerUIRefresh();
                    }, (err) => {
                        console.error("Curriculum sync error:", err);
                    });

                    // 2. Real-time Resources Sync Listener
                        ...
                        if (ALL_RESOURCES.length === 0) {
                            ...
                            for (let res of INITIAL_RESOURCES) {
                                ...
                                db.collection("resources").doc(res.id).set(res)
                                    .catch(e => console.error("Failed to seed default resource:", e));
                            }
                        }
                        triggerUIRefresh();
                    }, (err) => {
                    ...
                    // 3. Real-time Users Sync Listener
                    db.collection("users").onSnapshot((snapshot) => {
                        ...
                        if (USERS.length === 0) {
                            ...
                            for (let u of defaultUsers) {
                                db.collection("users").doc(u.username).set(u)
                                    .catch(e => console.error("Failed to seed default user:", e));
                            }
                        }
                        triggerUIRefresh();
                    }, (err) => {
                    ...
                    console.error("Firebase Firestore sync setup failed, running offline sandbox memory mode:", err);
                    this.setupOfflineMemoryMode();
                }
            } else {
                this.setupOfflineMemoryMode();
            }
    ...
    static setupOfflineMemoryMode() {
        CURRICULUM = CURRICULUM_DEFAULT;
        ALL_RESOURCES = [...INITIAL_RESOURCES];
        USERS = [
            { id: "user-1", username: "yashrohil07", name: "Yash Rohil", role: "Admin" },
            { id: "user-2", username: "amit_gju", name: "Amit Sharma", role: "Student" },
            { id: "user-3", username: "sneha_2026", name: "Sneha Goel", role: "Student" }
        ];
        setTimeout(triggerUIRefresh, 100);
    }
...
// 3.5 Global UI Live Trigger Refresh Handler
function triggerUIRefresh() {
    // 1. If on resource browser pages (notes.html or papers.html)
    if (document.getElementById("filesGrid")) {
        const pageType = (document.getElementById("notesLink") && document.getElementById("notesLink").classList.contains("active")) || window.location.pathname.includes("notes") ? "notes" : "paper";
        if (typeof renderResources === "function") {
            renderResources(pageType);
        }
    }
    // 2. If on homepage (index.html)
    if (document.getElementById("recentUploadsGrid")) {
        if (typeof renderRecentUploads === "function") renderRecentUploads();
        if (typeof renderTopRatedResources === "function") renderTopRatedResources();
        if (typeof updateStatsCounters === "function") updateStatsCounters();
    }
    // 3. If on analytics dashboard page (dashboard.html)
    if (document.getElementById("semesterStatsBars") || document.getElementById("totalResourcesDash")) {
        if (typeof initDashboardPage === "function") {
            initDashboardPage();
        }
    }
    // 4. If on admin dashboard (admin-dashboard.html)
    if (document.getElementById("ovrTotalFiles")) {
        const activeMenuItem = document.querySelector(".sidebar-menu .menu-item.active");
        if (activeMenuItem) {
            const activeTab = activeMenuItem.getAttribute("data-tab");
